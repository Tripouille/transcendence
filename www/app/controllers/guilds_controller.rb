    ...
    if (user_exists?)
      ...
      render json: @guild.as_json.merge({
        ...
        "invites" => ((user_is_guild_owner?) ? Invite.where(:guild_id => @guild[:id]) : {}).map { |invite| invite.as_json.merge({
          ...
          score: Match.where(status: "finished", challenged: false, winner: invite.user_id).size,
          }) }.as_json,
        "users" => User.where(:guild_id => @guild[:id]).select(:id, :username, :guild_id).with_otp.map { |user| user.as_json.merge({
          ...
          contribution: Match.where(status: "finished", :challenged => false).where(left_guild_id: @guild[:id], left_player: user.id).or(Match.where(right_guild_id: @guild[:id], right_player: user.id)).where(winner: user.id).select(:left_guild_id, :right_guild_id).distinct.size
          }) }.as_json
      }), status: :ok
    end
  end
